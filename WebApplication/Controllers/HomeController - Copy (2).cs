...
namespace WebApplication3.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        internal async Task<string> GetUserGuidByNameAsync(string Name)
        {
            UserProfile usr = await db.UserProfiles.FirstOrDefaultAsync(u => u.UserName == Name);
            return usr.Guid;
        }
        ...
        internal async Task<string> GetChatRoomGuidByTitleAsync(string Title)
        {
            ChatRoom chtr = await db.ChatRooms.FirstOrDefaultAsync(cr => cr.Title == Title);
            return chtr.Guid;
        }
        ...
        private Task<List<Message>> GetMessagesAsync()
        {
            string Guid = GetChatRoomGuidByTitleAsync("GlobalChatRoom").Result;
            return db.Messages.Where(m => m.Receiver == Guid).ToListAsync();
        }
        ...
        private Task<List<Message>> GetMessagesAsync(string Receiver)
        {
            string CurentUserGuid = GetUserGuidByNameAsync(User.Identity.Name).Result;
            return db.Messages.Where(msg => (msg.Sender == CurentUserGuid) & (msg.Receiver == Receiver) | (msg.Sender == Receiver) & (msg.Receiver == CurentUserGuid)).ToListAsync();
        }
        ...
        private Task<List<Message>> GetMessagesOfChatRoomAsync(string ChatRoomGuid)
        {
            return db.Messages.Where(m => m.Receiver == ChatRoomGuid).ToListAsync();
        }
        ...
        public async Task<ActionResult> ChatRoomAsync(string Receiver)
        {
            if (Receiver == db.ChatRooms.FirstOrDefault(cr => cr.Title == "GlobalChatRoom").Guid)
            {
                ViewBag.Messages = await GetMessagesAsync();
            }
            else if (db.UserProfiles.FirstOrDefault(u => u.Guid == Receiver) != null)
            {
                ViewBag.Messages = await GetMessagesAsync(Receiver);
            }
            else
            {
                ViewBag.Messages = await GetMessagesOfChatRoomAsync(Receiver);
            }

            //string TimeZoneId =  await db.UserProfiles.FirstOrDefaultAsync(u => u.UserName == User.Identity.Name).Result.TimeZoneId;
            UserProfile up = await db.UserProfiles.FirstOrDefaultAsync(u => u.UserName == User.Identity.Name);
            ViewBag.UserTimeZone = TimeZoneInfo.GetSystemTimeZones().FirstOrDefault(tz => tz.Id == up.TimeZoneId);
            ViewBag.ActivityDates = GetActivityDates(Receiver, GetUserGuidByName(User.Identity.Name));
